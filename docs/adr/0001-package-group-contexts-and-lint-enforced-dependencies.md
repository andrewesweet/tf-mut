...
`CONTEXT.md` is the single context map. The target dependency direction is
enforced by the configured linter: core, supporting, and Terraform Boundary
packages do not depend on Publication, the CLI, or renderers; `internal/report`
remains the stdlib-only publication leaf. The application layer coordinates the
contexts and projects their values into report DTOs.
...
not make an import violation fail when introduced. A hand-written architecture
test was rejected because the configured linter already answers this structural
question; duplicating it creates a second rule to maintain.
