...
| `internal/mutation` | The operator catalogue and its applicability matrix. Tier 0 is applied through `hclwrite`; Tiers 1–3 rewrite byte ranges, so a mutant differs from the original only in the tokens its operator owns; Tier 4 carries the three witnessed lifecycle operators, non-projecting and deep-tier only; Tier 5 is the two pack form operators, `PACK-FLIP` and `PACK-REPLACE`, parameterised by the entries of a pack loaded under the normative contract (`pack.go`: entry blocks only, literal values only, forms checked at load) and selected by name, never by tier; the shipped `security-aws` pack is embedded under its reserved name (`shipped.go`, `packs/security-aws.hcl`), parsed at init through the same contract, its reserved names derived from the embedded files, its entries required to state their provenance. Content-derived identifiers; deduplication unchanged, with every `(operator, pack, entry)` whose rewrite produced the surviving bytes aggregated onto the survivor as its origins, sorted and deduplicated by `(pack, entry)`; diffs |
