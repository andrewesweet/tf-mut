...
| `internal/mutation` | The operator catalogue and its applicability matrix. Tier 0 is applied through `hclwrite`; Tiers 1–3 rewrite byte ranges, so a mutant differs from the original only in the tokens its operator owns; Tier 4 carries the three witnessed lifecycle operators, non-projecting and deep-tier only. Content-derived identifiers; deduplication; diffs |
