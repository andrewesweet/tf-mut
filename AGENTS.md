...
| `internal/oracle` | The mutant outcome: state, diagnosis, message, fix and evidence behind unexported fields, constructible only through the survivor and terminal constructors whose parameters are each diagnosis's required evidence subset; the engine's precedence table is the constructors' only caller. The scored set, the headline metrics and the per-operator error rates are computed here, over outcomes |
