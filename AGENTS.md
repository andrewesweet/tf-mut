...
| `internal/discovery` | `hclsyntax` parsing of modules and `.tftest.hcl` files; the `..`-closure; provider and effect inventories; reference forms. The boundary publishes a representation-neutral expression contract: `Attribute.Expr` and `Validation.Condition` are `hcl.Expression`, and native syntax has two named accessors, both fail-closed: `NativeExpression`, and the second and last, `ReparsedNative`, through which a consumer that genuinely requires the tokens of a JSON declaration re-parses it at its own point of use |
| `internal/mutation` | The operator catalogue and its applicability matrix. Tier 0 is applied through `hclwrite`; Tiers 1–3 rewrite byte ranges, so a mutant differs from the original only in the tokens its operator owns; Tier 4 carries the three witnessed lifecycle operators, non-projecting and deep-tier only; Tier 5 is the three pack form operators, `PACK-FLIP`, `PACK-REPLACE` and `PACK-WIDEN-CIDR` (#176), parameterised by the entries of a pack loaded under the normative contract (`pack.go`: entry blocks only, literal values only, forms checked at load; `widen-cidr` pins both ends to the sentinel and the IPv4 any-prefix and its site rule is the IPv4 CIDR value rule in `operators_pack.go`) and selected by name, never by tier; the shipped `security-aws` pack is embedded under its reserved name (`shipped.go`, `packs/security-aws.hcl`), parsed at init through the same contract, its reserved names derived from the embedded files, its entries required to state their provenance. Content-derived identifiers; deduplication unchanged, with every `(operator, pack, entry)` whose rewrite produced the surviving bytes aggregated onto the survivor as its origins, sorted and deduplicated by `(pack, entry)`; diffs |
...
| `internal/oracle` | The Oracle / Verdict context: the mutant outcome, its diagnoses and their evidence behind unexported fields, constructible only through the survivor and terminal constructors whose parameters are each diagnosis's required evidence subset; the engine's precedence table and its cache rehydration are the constructors' only callers. The scored set, the headline metrics and the per-operator error rates are computed here, over outcomes |
| `internal/suggest` | The Suggestion context: the candidate-to-terminal lifecycle (`Candidate`, `Suggestion`, `Verification`, the closed `SkipReason` vocabulary — a verified outcome unspellable without its digest and both legs, a skipped one without a patch), the address, rendering and sensitivity adapters, the run-block patch writer, and the stable suggestion identity. The application layer projects the context's values onto `report.Suggestion` |
| `internal/characterise` | The generation direction: the scaffold planner, the mock and scenario renderer, the input-synthesis preference pipeline with its static validation evaluator, the granularity ladder and the pinning stage. It owns the judgement-point lifecycle — open, answered, promoted and rejected, reachable through transitions only, promotion only from an answered point that verified — the pin lifecycle — pinned, or skipped with a reason from its closed skip vocabulary and no expression, `PinSkipped` taking none — the construct-scaffold lifecycle — scaffolded, or promoted only through `Promote` carrying the verification that earned it, the artefact leaving with the promotion — and the scenario — identity, naming, state key and published inputs, derived by the planner and reachable through accessors only — plus the closed input-provenance vocabulary the preference order alone writes. Pins go through `internal/suggest`'s adapters unchanged, so a value that is unrenderable for one is unrenderable for both |
...
The JSON reporter's contract is published at `docs/schema/report-2.5.0.json` and validated in
the suite; `report-2.4.0.json`, `report-2.3.0.json`, `report-2.2.0.json`, `report-2.1.0.json`,
