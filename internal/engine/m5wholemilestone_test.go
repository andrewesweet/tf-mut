...
// schema-addition stamp `schema_version`, moved 2.3.0 to 2.4.0 by the pack
// work. With no pack selected, `origins` is absent from every mutant on
// either side, so the 2.4.0 schema's added field carries no content here.
//
...
func TestTheStandardReportOfTheMatrixFixtureIsInvariantAcrossTheWholeMilestone(t *testing.T) {
	...
	if after["schema_version"] != "2.4.0" {
		t.Fatalf("this tree publishes schema %q, want 2.4.0", after["schema_version"])
	}
