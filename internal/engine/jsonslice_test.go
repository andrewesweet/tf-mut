...
// TestAJSONDeclaredChildVariableAddsNoMutant holds the same promise where the
// inventories gate a mutant: a native call's input is deletable only when the
// child declares the variable, and a child that declares it in `.tf.json`
// must not turn reading that file into a `ModuleInputDelete` the unread
// closure does not have.
//
//nolint:paralleltest // owns package-global JSON-reading hook for its lifetime.
func TestAJSONDeclaredChildVariableAddsNoMutant(t *testing.T) {
	read := previewRequest(t, copyFixture(t, jsonChildVariableFixture))

	unread := previewRequest(t, copyFixture(t, jsonChildVariableFixture))
	engine.SetJSONReadingDisabled(t, unread.ModuleDir)
	...
	if strings.Join(verdicts(withJSON), "\n") != strings.Join(verdicts(withoutJSON), "\n") {
		t.Fatalf("reading the child's JSON variable changed the population:\nread:   %v\nunread: %v",
			verdicts(withJSON), verdicts(withoutJSON))
	}
}
